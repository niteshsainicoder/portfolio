...
function Card({ title,repoName,extratext="Working on the project",live,image}: CardProps) {
    ...
    const { ref, inView } = useInView({
        ...
        triggerOnce: false, // If you want the callback to fire only once
      });
    const setClicked = () => {
  ...
  setisEnter(!isEnter)
    ...
    return (
        <div ref={ref} onMouseEnter={() => { setisEnter(true) }} onMouseLeave={() => { setisEnter(false) }} className='relative overflow-hidden rounded-3xl '>
            <BackgroundGradient>
